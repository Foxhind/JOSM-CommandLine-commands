...
#
#       replace.py {Params}
#           stdin: {RefWay} {TargetWays}
#       
#       Script performs replacing geometry of target ways by reference way with care of existing nodes and tags
#       Params: 0 - do not perform rotation of objects, 1 - do perform
#       
#       Copyright 2011 Hind <foxhind@gmail.com>
#       modified by OverQuantum:
#       2014-12-06 added Params (0/1) - merging function of replace2.py; fix to 4 spaces indentation
#       
...
def main():
    ...
    rangle = 0
    if params == 1:
        rangle = getangle(FOREL(createtable(rData, nData, list(rData.ways.keys())[0]), 15))   #get main angle of reference object
...
def getangle(clusters):
    ...
    for cluster in clusters:
        ...
        if mass > maxmass:
            ...
            maxcluster = cluster
    angle = 0.0
    for segment in maxcluster:
        angle += segment[0]
    angle /= len(maxcluster)
    return angle
